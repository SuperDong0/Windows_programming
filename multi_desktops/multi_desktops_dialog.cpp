...
const TCHAR* g_desktop_names[] = {
        _T("default"),
        _T("multi_desktop_right"),
        _T("multi_desktop_top"),
        _T("multi_desktop_bottom"),
};
...
void WaitForOtherThreads()
{
        for (int i = 1; i < MAX_DESKTOPS_ALLOWED; i++) {
                ...
                // already exit, in that case the function will return
                // immediately.
...
{
        if (!RegisterHotKey(dialog, LEFT_DESKTOP, MOD_ALT, 'J'))
                PrintErrorWith(_T("RegisterHotKey fail!"));

        if (!RegisterHotKey(dialog, RIGHT_DESKTOP, MOD_ALT, 'L'))
                PrintErrorWith(_T("RegisterHotKey fail!"));
 
        if (!RegisterHotKey(dialog, TOP_DESKTOP, MOD_ALT, 'I'))
                PrintErrorWith(_T("RegisterHotKey fail!"));
 
        if (!RegisterHotKey(dialog, BOTTOM_DESKTOP, MOD_ALT, 'K'))
                PrintErrorWith(_T("RegisterHotKey fail!"));
}
